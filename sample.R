...
api_key <- "pWjKHzVNwggRSx4M0ysGRtZtI"
api_secret <- "eMlXwUCASMoFhE33aEnsc88wnvwFeWuy96zmjmPxzkBcdmyVTF"
access_token <- "134522082-Hpi9Z9AQcpq7PDm6iFRqmb4vIVRrQehik5MPmemP"
access_token_secret <- "3mZugI3RmpPI0ihpyAZIk3wnUGChpNdkg1Tclnv7YfBPg"
...
bjp_tweets = searchTwitter("trump", n=1000, lang="en")
...
p <- ggplot(sentiment_dataframe, aes(x=emotion))
p=p+geom_bar(aes(y=..count.., fill=emotion)) 
p=p+  scale_fill_brewer(palette="Dark2")  
p=p+  ggtitle('Sentiment Analysis of Tweets on Twitter about BJP') 
p=p+  theme(legend.position='right') 
p=p+  ylab('Number of Tweets') 
p=p+  xlab('Emotion Categories') 
p + geom_point()
print(p)
# Similary we will plot distribution of polarity in the tweets
q=ggplot(sentiment_dataframe, aes(x=polarity))
q=q+geom_bar(aes(y=..count.., fill=polarity))
  q=q+scale_fill_brewer(palette="RdGy")
  q=q+ggtitle('Sentiment Analysis of Tweets on Twitter about BJP')
  q=q+theme(legend.position='right')
  q=q+ylab('Number of Tweets') 
  q=q+xlab('Polarity Categories')
  print(q)
  ...
  # Now let us create a corpus which computes and represent words on corpora (corpora are collections of documents containing (natural language) text). For more please look at help on Corpora under "tm" package.
